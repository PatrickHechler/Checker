 ...
 * <p>
 * a {@link Result} is a {@link #goodResult()} when the method returned
 * normally.<br>
 * the normal return value will be saved in the field {@link #result} and can be
 * accessed via
 * {@link #getResult()}. <br>
 * a {@link Result} is a {@link #badResult()} when the method ended by throwing
 * a
 * {@link Throwable}.<br>
 * the {@link Throwable} will be saved in the field {@link #err} and can be
 * accessed via
 * {@link #getErr()}.
 * <p>
 * additional to the return value or the thrown value the {@link Result} also
 * saves the execution
 * time, needed for the invoked method.<br>
 * the start and end time can be accessed via {@link #start} and
 * {@link #end}.<br>
 * to get the time, which had been needed to execute the method the
 * {@link #getTime()} method can be
 * used.
 * 
 * @author Patrick
 */
public final class Result {
	...
	/**
	 * this saves the return value (or <code>null</code> if it was a
	 * <code>void</code> method)<br>
	 * if the method did not end normally (it throwed an {@link Throwable}) this
	 * value will be
	 * <code>null</code>
	 */
	private final Object    result;
	/**
	 * here will be the {@link Throwable} which was thrown by the called
	 * {@link Method}<br>
	...
	/**
	 * creates a {@link Result} with the {@link Object} as return value of a
	 * {@link Method}
	 * 
	 * @param met   the executed method
	 * @param e     the return value of the executed {@link Method}
	...
	/**
	 * creates a {@link Result} with the {@link Throwable} saved as error of a
	 * called {@link Method}
	 * 
	 * @param met   the failed method
	 * @param t     the {@link Throwable} thrown while the {@link Method} was called
	 * @param start the start time of the execution
	 * @param end   the end time of the execution
	 */
	...
	/**
	 * if this is a 'good' {@link Result} this {@link Method} will return
	 * <code>true</code>.<br>
	 * This is a 'good' {@link Result} if no {@link Throwable} was thrown during the
	 * method call or in
	 * other words if <code>{@link #err} == null</code> is <code>true</code>
	 * 
	 * @return <code>true</code> if this {@link Result} is a 'good' {@link Result},
	 *         <code>false</code>
	 *         if not
	 */
	public boolean goodResult() { return this.err == null; }
	
	/**
	 * if this is a 'bad' {@link Result} this {@link Method} will return
	 * <code>true</code>.<br>
	 * This is a 'bad' {@link Result} if a {@link Throwable} was thrown during the
	 * method call or in
	 * other words if <code>{@link #err} != null</code> is <code>true</code>
	 * 
	 * @return <code>true</code> if this {@link Result} is a 'bad' {@link Result},
	 *         <code>false</code> if
	 *         not
	 */
	public boolean badResult() { return this.err != null; }
	
	/**
	 * returns the saved return value of the {@link Method}.<br>
	 * if this is no {@link #goodResult() good Result} it will throw a
	 ...
	 * if this is no {@link #badResult() bad Result} it will throw a
	 * {@link NoSuchElementException}
	 * 
	...
	/**
	 * prints a detailed represention of this {@link Result}, which contains even
	 * some informations
	 * which are not saved by this {@link Result} (then the information comes form
	 * the
	 * {@link Method} {@code me}), when {@code me} is <code>null</code> these
	 * informations are skipped
	 * 
	 * @param out
	 *                the {@link PrintStream} on which this {@link Result} should be
	 *                printed
	 * @param me
	 *                the {@link Method} of this {@link Result} with additional
	 *                information. ignored if
	 *                <code>null</code>
	 * @param params
	 *                the parameter values used to generate this {@link Result}
	 * @param indent
	 *                the normal indention
	 * @param dindent
	 *                the complete indention of double indented lines
	 */
	public void detailedPrint(PrintStream out, Method me, Object[] params, String indent, String dindent) {
		if (me != null) {
			out.print(indent + "checked method: " + me.getName() + "(");
			Parameter[] paramTypes = me.getParameters();
			if (paramTypes.length > 0) {
				Annotation[] a = paramTypes[0].getAnnotations();
				for (int i = 0; i < a.length; i++) {
					Class<? extends Annotation> acls = a[i].annotationType();
					out.print("@" + acls.getSimpleName() + ' ');
				}
				out.print(paramTypes[0].getType().getName());
				if (paramTypes[0].isNamePresent()) {
					out.print(" " + paramTypes[0].getName());
					out.print(" '" + params[0] + '\'');
				}
				for (int i = 1; i < paramTypes.length; i++) {
					out.print(", ");
					a = paramTypes[i].getAnnotations();
	...
	/**
	 * this method returns a {@link String} with the {@link #err}or or
	 * {@link #result} of this
	 * {@link Result}. There will be no Prefix or Postfix
	 * 
	 * @return a {@link String} with the {@link #err} or {@link #result} of this
	 *         {@link Result} without
